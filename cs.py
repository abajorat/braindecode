...
def create_config_space():
    ...
    filter_length_4 = CS.UniformIntegerHyperparameter("filter_length_4",
                                                  ...
                                                  upper=14,
                                                  log=False)
    first_nonlin = CS.CategoricalHyperparameter("first_nonlin", ['elu', 'relu6', 'relu', 'tanh'])
    first_pool_mode = CS.CategoricalHyperparameter("first_pool_mode", ['max', 'mean'])
    first_pool_nonlin = CS.CategoricalHyperparameter("first_pool_nonlin", ['identity', 'safe_log'])
    later_nonlin = CS.CategoricalHyperparameter("later_nonlin", ['elu', 'relu6', 'relu', 'tanh'])
    later_pool_mode = CS.CategoricalHyperparameter("later_pool_mode", ['max', 'mean'])
    later_pool_nonlin = CS.CategoricalHyperparameter("later_pool_nonlin", ['identity', 'safe_log'])
    drop_prob = CS.UniformFloatHyperparameter("drop_prob",
                                                  lower=0.0,
                                                  default_value=0.5,
                                                  upper=0.9,
                                                  log=False)
    split_first_layer = CS.CategoricalHyperparameter("split_first_layer",['True'] )
    batch_norm = CS.CategoricalHyperparameter("batch_norm",['True', 'False'] )
    stride_before_pool = CS.CategoricalHyperparameter("stride_before_pool",['False'] )
